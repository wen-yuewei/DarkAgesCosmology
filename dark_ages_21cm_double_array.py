...
def circular_pdf(d, D_min, D_max):
    """Radial PDF of baseline length within one disk (diameter D)."""
    d = np.asarray(d)
    pdf = np.zeros_like(d)
...
def dN_intra_dd(d, N, D_min, D_max):
    """Intra-array number density per metre (1D)."""
...
def f_cross_length(d_grid, L, D_min, D_max, n_theta=1000):
    """
    d_grid : 2D array
    L, D_min, D_max : floats
    n_theta : number of quadrature points
    Returns array of same shape as d_grid.
    """
    # Create theta points and integration weights (Simpson's rule)
    theta = np.linspace(0, 2*np.pi, n_theta)
    # dtheta constant for uniform spacing, Simpson weights applied later

    # Reshape d_grid to (1,1,m,n) or similar to broadcast against theta
    # We want: r.shape = (len(theta), m, n)
    # Use broadcasting: theta (T,) and d_grid (M,N) -> r (T,M,N)
    cos_theta = np.cos(theta).reshape(-1, 1, 1)   # shape (T,1,1)
    d_reshaped = d_grid.reshape(1, *d_grid.shape) # shape (1,M,N)

    r = np.sqrt(d_reshaped**2 + L**2 - 2 * d_reshaped * L * cos_theta)

    # Avoid division by zero: where r is tiny, set integrand to 0
    # Assuming circular_pdf can handle arrays
    pdf_vals = circular_pdf(r, D_min, D_max)   # should be elementwise
    integrand_vals = np.where(r < 1e-12, 0.0, pdf_vals / (2 * np.pi * r))

    # Integrate over theta using Simpson's rule along axis=0 (the theta axis)
    integral = simpson(integrand_vals, x=theta, axis=0)

    return d_grid * integral

def dN_cross_dd(d, L, N, D_min, D_max):
    """Cross-baseline number density per metre (1D)."""
    # Only non-zero for d between |L-D| and L+D
...
def dN_total_dd(d, L, N, D_min, D_max):
    """Total 1D number density (intra + cross)."""
    return dN_intra_dd(d, N, D_min, D_max) + dN_cross_dd(d, L, N, D_min, D_max)

def nb_D_tot(d, L, N, D_min, D_max):
    epsilon = 1e-10   # small compared to typical density values
...
def noise_power_spectrum(k_perp):
    ...
    nb_D = nb_D_tot(D_val, L, N_antenna, D_min, D_max)
...
k_perp_max = 2 * np.pi * (D_max + L) / (r_z * wavelength)
...
def run():
    ...
    np.savez('outputs_double_array.npz',
